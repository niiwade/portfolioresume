    ...
    <Bounded as="footer" className="text-slate-600">
      <div className="container mx-auto mt-20 flex flex-col items-center justify-between gap-6 py-8 sm:flex-row ">
        <div className="name flex flex-col items-center justify-center gap-x-4 gap-y-2 sm:flex-row sm:justify-self-start">
          <Link
            href="/"
            className="text-xl font-extrabold tracking-loose text-slate-100 transition-colors duration-150 hover:text-green-400"
          >
            {settings.data.name}
          </Link>
          <span
            className="hidden text-3xl font-extralight leading-[0] text-slate-400 sm:inline"
            aria-hidden={true}
          >
            |
          </span>
        ...
        <div className="socials inline-flex justify-center sm:justify-end">
          ...
          {isFilled.link(settings.data.email) && (
            <PrismicNextLink
              field={settings.data.email}
              className="p-2 text-2xl text-slate-300 transition-all duration-150 hover:scale-125 hover:text-green-400"
              aria-label={settings.data.name + " via Email"}
            >
              <MdOutlineMail/>
            </PrismicNextLink>
          )}
